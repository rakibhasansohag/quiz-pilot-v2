...
export async function createSession({
	...
	// create TTL index on expiresAt if not exists (safe to call repeatedly)
	try {
		await sessions.createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 });
	} catch (e) {
		// ignore index-creation errors (already exists or permissions)
	}
...
export async function enforceDeviceLimit(
	...
	const toRemove = active.slice(maxDevices); // oldest by lastSeenAt (since sorted desc)
